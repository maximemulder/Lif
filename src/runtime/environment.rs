...
fn primitive_assert<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn primitive_error<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let reference = engine.call_method(arguments[0], "to_string", Vec::new());
	println!("{}", reference.read().data_string());
	panic!();
}

fn primitive_exit<'a>(_: &mut Engine<'a>, _: Vec<GcValue<'a>>) -> GcReference<'a> {
	panic!();
}

fn primitive_new<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_instance(arguments[0]);
}

fn primitive_print<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let reference = engine.call_method(arguments[0], "to_string", Vec::new());
	println!("{}", reference.read().data_string());
	return engine.new_undefined();
}

fn array_to_string<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn array_copy<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn array_remove<'a>(engine: &mut Engine<'a>, mut arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let index = *arguments[1].data_integer();
	arguments[0].data_array_mut().remove(index);
	return engine.new_undefined();
}

fn array_access<'a>(_: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return arguments[0].data_array()[*arguments[1].data_integer()];
}

fn boolean_to_string<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_string(arguments[0].data_boolean().to_string());
}

fn boolean_comparison<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_boolean(arguments[0].data_boolean() == arguments[1].data_boolean());
}

fn class_to_string<'a>(engine: &mut Engine<'a>, _: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_string("CLASS".to_string());
}

fn class_chain<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn instance_chain<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let name = arguments[1].data_string().clone();
	let mut this = arguments[0];
	if let Some(method) = this.get_method(engine, &name) {
		engine.this = Some(this);
...
fn integer_to_string<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_string(arguments[0].data_integer().to_string());
}

fn integer_comparison<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_boolean(*arguments[0].data_integer() == *arguments[1].data_integer());
}

fn integer_lesser<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_boolean(*arguments[0].data_integer() < *arguments[1].data_integer());
}

fn integer_addition<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_integer(*arguments[0].data_integer() + *arguments[1].data_integer());
}

fn integer_subtraction<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_integer(*arguments[0].data_integer() + *arguments[1].data_integer());
}

fn integer_multiplication<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_integer(*arguments[0].data_integer() + *arguments[1].data_integer());
}

fn integer_division<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_integer(*arguments[0].data_integer() / *arguments[1].data_integer());
}

fn integer_remainder<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_integer(*arguments[0].data_integer() % *arguments[1].data_integer());
}

fn object_comparison<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	return engine.new_boolean(arguments[0] == arguments[1]);
}

fn object_difference<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let reference = engine.call_method_self(arguments[0], "==", arguments);
	return engine.new_boolean(!reference.read().data_boolean());
}

fn object_greater<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn object_greater_equal<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let reference = engine.call_method_self(arguments[0], "<", arguments);
	return engine.new_boolean(!reference.read().data_boolean());
}

fn object_lesser_equal<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
	let left  = engine.call_method_self(arguments[0], "<", arguments.clone());
	let right = engine.call_method_self(arguments[0], "==", arguments.clone());
	return engine.new_boolean(*left.read().data_boolean() || *right.read().data_boolean());
}

fn object_chain<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
...
fn string_concatenation<'a>(engine: &mut Engine<'a>, arguments: Vec<GcValue<'a>>) -> GcReference<'a> {
