...
fn eval<'a>(engine: &mut Engine<'a>, arguments: Arguments<'a>) -> ReturnReference<'a> {
    let code = Code::from_string(&engine.grammar, 1, &build::expression, &arguments[0].data_string());
...
fn exec<'a>(engine: &mut Engine<'a>, arguments: Arguments<'a>) -> ReturnReference<'a> {
    let code = Code::from_string(&engine.grammar, 0, &build::program, &arguments[0].data_string());
    engine.run(code);
    Ok(engine.undefined())
}
...
fn include<'a>(engine: &mut Engine<'a>, arguments: Arguments<'a>) -> ReturnReference<'a> {
    engine.run_frame(engine.scope().parent().unwrap(), |engine| {
        let code = Code::from_file(&engine.grammar, 0, &build::program, &arguments[0].data_string()).unwrap();
        engine.run(code);
    });
